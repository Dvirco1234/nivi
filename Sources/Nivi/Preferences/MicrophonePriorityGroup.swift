...
///
/// Nivi walks the list from the top and records from the first device that is plugged
        ...
        Group {
            if order.isEmpty {
                PrefEmptyState(icon: "mic.slash",
                               ...
                               message: "Plug one in, or check that Nivi is allowed to use the microphone.")
            } else {
        ...
        return HStack(spacing: PrefTheme.rowIconGap) {
            ...
            if id == inUseID {
                Circle()
                    ...
                    .help("Nivi records from this microphone")
            } else if !isConnected {
